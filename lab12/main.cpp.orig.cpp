...
ISR(TIMER1_CAPT_vect)
{
    ...
    if (!bitCheck(TCCR1B, ICES1))
    {
        ...
        // Comparator output high duration
        tHigh = tFall - tRise;
    }
    else
    {
        ...
        // Comparator output low duration
        tLow = tRise - tFall;
    }

    // Toggle between rising-edge and falling-edge capture
...
int main(void)
{
    ...
    float dc_est = 0;
    float dc_true = (OCR2B + 1.0) / (OCR2A + 1.0) * 100.0;

    float f_est = 0;
    float f_true = 16000000.0 / (1024.0 * (OCR2A + 1.0));
...
void set_tc1(void)
{
    /*
        ...
        The Analog Comparator output is connected to the TC1
        Input Capture unit by setting ACIC in ACSR in main().
    */
