...
void config_tc0()
{
    ...
    // Fast PWM, TOP = OCR0A, non-inverting output on OC0B
    // WGM02:0 = 111
    // COM0B1:0 = 10  -> clear OC0B on compare match, set at BOTTOM
    TCCR0A = 0b00100011;   // COM0B1=1, WGM01=1, WGM00=1
    TCCR0B = 0b00001000;   // WGM02=1, prescaler bits added later by start_tc0

    OCR0A = 199;           // TOP
    OCR0B = 10;            // initial duty

    // Timer0 overflow period in microseconds for current TOP
...
int main(void)
{
    ...
    bitSet(DDRD, pin_oc0b);          // PD5 / OC0B as output
    ...
    while (1)
    {
        ...
        //OCR0B = (uint8_t)fun_map(D, 10, 300, OCR0A - 1, 10);
