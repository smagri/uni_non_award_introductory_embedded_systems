...
void config_tc0()
{
    ...
    // Fast PWM, TOP = OCR0A, non-inverting on OC0B
    TCCR0A = 0b00100011;   // COM0B1=1, WGM01=1, WGM00=1
    TCCR0B = 0b00001000;   // WGM02=1 (TOP = OCR0A), prescaler set later

    OCR0A = 199;   // TOP (sets PWM frequency)
    OCR0B = 10;    // initial duty

    Tov = (OCR0A + 1) / clock_tc0 * 1.0e6;
...
int main(void)
{
    ...
    bitSet(DDRD, pin_oc0b);          // PD6 / OC0A as output
