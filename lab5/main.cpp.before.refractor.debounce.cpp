...
void debounce(volatile uint8_t *ddr_switch, volatile uint8_t *port_switch,
...
int main(void)
{
    ...
    while (1) {
        ...
        // Debounce the switch.
        debounce(ddr_switch, port_switch, port_switch_inputs,
              ...
              bool *system_on_off_toggle)
{
    ...
    // What value  is on the input  pin PD3, where we  have the button
    // connected.  It is intially high.   Wait for the transition from
    // high-low  to  toggle  the  LED, changing  its  state  from  its
    // previous state.
    ...
    if(switch_status != switch_status_old){
        ...
        if (switch_status != switch_status_old){
...

            {
                ...
                // We only toggle on the  press edge.  Not on both
                // edges high-low and low-high.
                if(switch_status == 0){
