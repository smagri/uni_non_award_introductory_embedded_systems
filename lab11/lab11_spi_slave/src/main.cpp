...
int main(void){
    ...
    // Interface the adc/thermistor to the atmega328p ports.
    ...
    //
    float temperature_master = 0;
    float temperature_slave = 0;
    ...
    while (1) {
        ...
        // ADC register values are proportional to thermistor temperatures.
        ...
        temperature_slave = linear_mapping1(adc, adc_min, adc_max,
        ...
        if (slave_received_master_byte) {
            ...
            usart_send_string(">temperature@master@SLAVE:");
...
void spi_master_init(void){
    ...
    // MIS0 = Master In Slave Output
    // SS = Slave Select
    // SLK = Clock

    // Port configuration:
    //
    // PB3 = MOSI, PB5 = SCK, PB2 = SS as outputs
    DDRB |= (1 << PB3) | (1 << PB5) | (1 << PB2);
    //DDRB |= (1 << PB3) | (1 << PB5);
    ...
    // Keep slave deselected initially, ie SS high
    //PORTB |= (1 << PB2);

    // SPI configuration:
    //
    // Enable SPI, Master mode, clock = F_CPU/16, SPI Interrupt Enable
